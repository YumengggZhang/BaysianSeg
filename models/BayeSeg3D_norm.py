...
from efficientunet_norm import get_efficientunet_b2

from .Basic_module import Criterion, Visualization
from .ResNet3D_norm import ResNet_appearance, ResNet_shape
...
class BayeSeg_Criterion(Criterion):
    ...
    def forward(self, pred, grnd):
        loss_dict = {
            ...
            "loss_Bayes": self.bayes_loss_coef * self.loss_Bayes(pred),
            "rho": torch.mean(pred["rho"]),
            "omega": torch.mean(pred["omega"]),
            "upsilon": torch.mean(pred["upsilon"]),
        }
        losses = (
            loss_dict["loss_Dice_CE"] + loss_dict["loss_Bayes"]
        )
