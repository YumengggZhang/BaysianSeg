...
class Trainer:
    def __init__(self, args):
        self.args = args
        # args.resume = "./logs/model/try/checkpoint.pth"
        args.resume = None
        args.dataset = 'ImageCAS3d'
        args.batch_size = 1
        args.model = 'BayeSeg3d'
        args.epochs = 500
        args.output_dir = "./logs/model/try"
        args.lr = 5e-5
        self.output_dir = Path(args.output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)

        self.logger = get_logger(name="BayeSeg3d", root=self.output_dir)
        ...
        # Adjust as needed (e.g., 2, 4, 8, etc.)
        # ------------------------------------------------------
        self.accumulate_grad_steps = 4
    ...
    def train_one_epoch(self):
        ...
        # ZERO GRAD before starting accumulation
        ...
        for step in range(total_step):
            ...
            # Mixed precision for forward pass and loss
            with torch.autocast("cuda", dtype=torch.bfloat16):
                outputs = self.model(samples)
                losses, loss_dict = self.criterion(outputs, targets)
                losses = losses / self.accumulate_grad_steps
            ...
            # Scale the loss and backprop (no step yet)
            losses.backward()

            # Gradient accumulation: only step every 'accumulate_grad_steps' iteration
            if (step + 1) % self.accumulate_grad_steps == 0 or (step + 1) == total_step:
                # Unscale grads, clip, and step
                
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), 1.0)
                self.optimizer.step()
                self.optimizer.zero_grad()

                # ZERO GRAD again after stepping
    ...
    def evaluate(self, save):
        ...
        for step in range(total_step):
            ...
            # Optionally save predicted segmentation results
            if step % (max(round(total_step / 16.0), 1)) == 0:
                ...
                if save:
                    save_path = os.path.join(
                        "./logs/model/unet_baye_2/segmentation_result",
                        f"target_segmentation_{step}.nii"
                    )
