...
class Trainer:
    def __init__(self, args):
        self.args = args
        # args.resume = r'./logs/model/unet_baye_bf16/checkpoint.pth'
        args.resume = None
        args.dataset = 'ImageCAS3d'
        args.batch_size = 1
        args.model = 'BayeSeg3d'
        # args.epochs = 60
        args.output_dir = "./logs/model"
        args.lr = 5e-5
        ...
        # Initialize model, criterion, and visualizer
    ...
    def evaluate(self, save):
        ...
        for step in range(total_step):
            ...
            # Use autocast during evaluation for consistency
            # with torch.cuda.amp.autocast():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                outputs = self.model(samples)
                losses, loss_dict = self.criterion(outputs, targets)
