...
class Trainer:
    def __init__(self, args):
        self.args = args
        args.resume = r'./logs/model/unet_baye_256_64_accum/checkpoint.pth'
        # args.resume = None
        args.dataset = 'ImageCAS3d'
        args.batch_size = 1
        args.model = 'BayeSeg3d'
        args.epochs = 60
        args.output_dir = "./logs/model/unet_baye_256_64_accum"
    ...
    def evaluate(self,save):
        ...
        for step in range(total_step):
            ...
            if step % (max(round(total_step / 16.0), 1)) == 0:
                ...
                target_np = target_tensor.cpu().numpy()  # Convert to NumPy

                target_sitk = sitk.GetImageFromArray(target_np)  # Converts [D, H, W] → SimpleITK Image

                if save:
                    # Define file path to save
                    save_path = os.path.join("./logs/model/unet_baye_2/segmentation_result",f"target_segmentation_{step}.nii")
